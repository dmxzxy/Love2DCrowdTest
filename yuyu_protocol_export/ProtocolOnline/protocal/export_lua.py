...
def do_package_lua(context,output_path):
    ...
    for module in modules:
        ...
        for protocal in protocals:
            ...
            prefix = "Protocals" #for yuyu project
            file_lua.write('-- @desc  ')
            file_lua.write(protocal.desc.encode('utf-8'))
            file_lua.write('\n')
            segments = protocal.segments
            for s in segments:
            	file_lua.write('-- @field %-30s  %-20s @'%(s.type.name, s.name, ))
            	file_lua.write(s.desc.encode('utf-8'))
            	file_lua.write('\n')
    ...
    if needUnfold:
	    ...
	    for module in modules:
	        ...
	        for protocal in protocals:
	            ...
	            segments = protocal.segments
	            for s in segments:
	                segment_type_name = s.type.name
	                names = segment_type_name.split('.')
	                segment_type_name = names[len(names)-1]
	                if s.type.is_basic:
	                    if s.protocal_type.name == "repeated":
	                        file_lua.write('    msg.%s = luaList(msg.%s);'%(s.name,s.name))
	                    else:
	                        file_lua.write('    msg.%s = %s;'%(s.name, s.name))
	                    file_lua.write('    -- @')
	                    file_lua.write(s.desc.encode('utf-8'))
	                    file_lua.write('\n')
	                else:
	                    if s.protocal_type.name == "repeated":
	                        file_lua.write('    msg.%s = luaList(msg.%s, %s.unfold%s);'%(s.name,s.name,prefix,segment_type_name))
	                    else:
	                        file_lua.write('    msg.%s = %s.unfold%s(%s);'%(s.name,prefix,segment_type_name,s.name))
	                    file_lua.write('    -- @')
	                    file_lua.write(s.desc.encode('utf-8'))
	                    file_lua.write('\n')
